...
import { getEnquiry, getEnquiryLookups } from '@/lib/enquiries';
import { getClientsBasic, getClientActivities, getActivityTypes } from '@/lib/clients';
import { getPropertiesBasic } from '@/lib/properties';
import { getClientTasks, getTaskTypes } from '@/lib/tasks';
import { updateEnquiryAction, deleteEnquiryAction } from '@/app/enquiries/actions';
...
export default async function EnquiryDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [enquiry, lookups, clients, properties] = await Promise.all([
    getEnquiry(id),
    getEnquiryLookups(),
    getClientsBasic(),
    getPropertiesBasic(),
  ...
  const [activities, activityTypes, tasks, taskTypes] = await Promise.all([
    getClientActivities(enquiry.client_id),
    getActivityTypes(),
    getClientTasks(enquiry.client_id),
    getTaskTypes(),
  ]);

  const linkedProperty = enquiry.property_id ? properties.find((p) => p.id === enquiry.property_id) ?? null : null;

  const updateWithId = updateEnquiryAction.bind(null, id);
  const deleteWithId = deleteEnquiryAction.bind(null, id);
        ...
        <div className="mt-3 flex items-start justify-between">
          <div>
            <h1 className="text-2xl font-semibold text-zinc-100">
              {enquiry.clients ? (
                <Link href={`/clients/${enquiry.clients.id}`} className="hover:text-blue-300">
